/*************************************************************************
	...
	> Created Time: Fri 08 Jul 2016 02:53:03 PM CST
 ************************************************************************/
#include <sync.h>
#include <string.h>
#include <assert.h>
...
// 根据输入参数 addr 和 mm 变量,查找在 mm 变量中的 mmap_list 双向链表中某个 vma 包含此add.
struct vma_struct *
...
check_vma_overlap(struct vma_struct *prev, struct vma_struct *next) 
{
    ...
    // cprintf("prev vma [%08lx]~[%08lx]\n",prev->vm_start,prev->vm_end);
    // cprintf("next vma [%08lx]~[%08lx]\n",next->vm_start,next->vm_end);
}


// insert_vma_struct -insert vma in mm's list link
// 将一个 vma 按照其空间位置 [vma->vm_start , vma->vm_end] 从小到大的顺序插入到所属的 mm 中
// 的 mmap_list 双向链表中. 
void
...
do_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr) 
{
    ...
    if (vma == NULL || vma->vm_start > addr) {
        cprintf("not valid addr [0x%08lx], and  can not find it in vma\n", addr);
        goto failed;
    }
    ...
    if (*ptep == 0) { 
        ...
        cprintf("do pgfault: ptep [0x%08lx], pte [0x%08lx]\n",ptep, *ptep);
    ...
    if (mm != NULL) {
        ...
        while (start < end) {
            ...
            if (write && (vma->vm_flags & VM_STACK)) {
                if (start < vma->vm_start + PAGE_SIZE) { //check stack start & size
                    return 0;
                }
            }
            start = vma->vm_end;
